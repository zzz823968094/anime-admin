<template>
  <div class="access-user-detail">
    ...
    <div class="card">
      <div class="chart-header">
        <h2 class="sec-title tile-heading">
          <svg style="width: 24px; height: 24px; margin-right: 8px; vertical-align: middle;" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M3 3v18h18"></path><path d="M18.7 8l-5.1 5.2-2.8-2.7L7 14.3"></path></svg>
          用户留存率分析
        </h2>
        <input
      ...
      <div v-else-if="!retentionData || Object.keys(retentionData).length === 0" class="loading-text">暂无留存数据</div>
      <div v-else class="retention-grid">
        <div
          v-for="(rate, days) in retentionData"
          :key="days"
          class="stat-card retention-item"
        >
          <div>
    ...
    <div class="card">
      <div class="chart-header">
        <h2 class="sec-title tile-heading">
          <svg style="width: 24px; height: 24px; margin-right: 8px; vertical-align: middle;" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"></circle><line x1="2" y1="12" x2="22" y2="12"></line><path d="M12 2a15.3 15.3 0 0 1 4 10 15.3 15.3 0 0 1-4 10 15.3 15.3 0 0 1-4-10 15.3 15.3 0 0 1 4-10z"></path></svg>
          地理位置分布
        </h2>
      ...
      <div v-else-if="!locationData || locationData.length === 0" class="loading-text">暂无地理位置数据</div>
      <div v-else>
        <!-- 图表可视化区域 - 始终挂载但通过 v-show 控制 -->
        <div ref="mapContainer" class="chart-container" v-show="!loadingLocation && locationData.length > 0"></div>
        ...
        <table class="tbl" style="margin-top: 24px;">
          <thead>
            <tr>
              ...
              <th>省份</th>
              <th>城市</th>
              <th @click="sortBy('userCount')" style="cursor: pointer;">用户数</th>
              <th @click="sortBy('ipCount')" style="cursor: pointer;">IP数</th>
              <th>占比</th>
            </tr>
          </thead>
          <tbody>
            <tr v-for="(row, index) in sortedLocationData" :key="index">
              ...
              <td>
                <div class="progress-bar-container" style="height: 6px;">
                  <div 
                    class="progress-bar-fill" 
                    :style="{ width: getLocationPercentage(row.userCount) + '%', backgroundColor: '#0071e3' }"
                  ></div>
                </div>
                <span class="caption" style="margin-top: 4px; display: block;">{{ getLocationPercentage(row.userCount) }}%</span>
              </td>
            </tr>
          </tbody>
        </table>
...
const getRateClass = (rate) => {
  if (rate >= 50) return 'rate-excellent'
...
/**
 * 计算地理位置占比
 * @param {number} userCount - 用户数
 * @returns {number} 百分比
 */
const getLocationPercentage = (userCount) => {
  if (!locationData.value || locationData.value.length === 0) return 0
  
  const totalUsers = locationData.value.reduce((sum, item) => sum + (item.userCount || 0), 0)
  if (totalUsers === 0) return 0
  
  return ((userCount / totalUsers) * 100).toFixed(2)
}
...
const sortedLocationData = computed(() => {
  if (!sortKey.value) return locationData.value
...
const renderMap = (retryCount = 0) => {
  ...
  const container = document.querySelector('.access-user-detail .chart-container') || document.querySelector('.chart-container')
    ...
    data: provinces.map(prov => {
      if (city === '其他') {
        ...
        return locationData.value
          .filter(item => item.province === prov && otherCities.includes(item.city || item.province))
          .reduce((sum, item) => sum + item.userCount, 0)
      }
      const item = locationData.value.find(d => d.province === prov && (d.city || d.province) === city)
